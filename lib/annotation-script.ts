...
/**
 * Vanilla-JS XPath/rect helpers shared between the artifact iframe's
 * annotation bridge (below) and the External Reviews widget
 * (lib/review-widget/source.ts). Kept as a plain string (not a real module)
 * because both consumers inline it into a `<script>` tag / standalone JS file
 * that runs outside any bundler — there's no build step for either.
 */
export const XPATH_HELPERS_SRC = `
...
export const REDUCED_MOTION_FALLBACK = `<style>
@media(prefers-reduced-motion:reduce){
  *{opacity:1!important;transform:none!important;visibility:visible!important}
}
</style>`;

export const ANNOTATION_SCRIPT = `<script>
(function(){
  // === HEIGHT REPORTING ===
  function report(){
