<!-- Thanks for contributing to Artiline. -->

## What & why

<!-- What does this change and why? Link any related issue (Closes #123). -->

## How it was tested

<!-- Commands run, manual steps, screenshots for UI. -->

## Checklist

- [ ] Commits are signed off (`git commit -s`) per the DCO
- [ ] `pnpm tsc --noEmit`, `pnpm lint`, and `pnpm build` pass
- [ ] Schema changes include generated migrations (`pnpm db:generate`, committed)
- [ ] New cloud/paid features are dynamic-imported behind `isFeatureEnabled()`
- [ ] Docs / env matrix updated if behavior or configuration changed
- [ ] Conventional Commits format used for the title (`feat:`, `fix:`, ...)
